...
All formats are versioned `1.2.0`, right-handed, Y-up, and meter-based. Readers migrate the additive `1.0.0` and `1.1.0` representations before validation without rewriting immutable snapshots.
...
The deterministic DAG performs requirements planning, polygon rasterization, compiled landforms/splats, seeded features/scatter, regional composition, calibrated placement, fixed Blender region refinement, structured visual review, optimization and immutable export. Local produces the three canonical artifacts as a draft. Cheap adds structured planning/review, a known-camera terrain edit, an isolated reference and WaveSpeed single-image Tripo. Experimental Studio detects objects in the actual composition, creates local SAM2.1 Large masks and lossless crops, synthesizes the ordered front/left/back/right set, sends it only to the exact WaveSpeed Tripo H3.1 multiview PBR adapter, validates candidates through a separate allowlisted Blender 5.1 process, records RGB/depth/normal/semantic/instance passes, and applies bounded mesh-footprint support edits. Every descriptor, crop affine, box, projection, pass and atlas tile retains the calibrated camera provenance.

Each compile owns a permanent content-addressed artifact catalog and attempt history. Reviewed policy unit cost is reserved immediately before every unique provider action; the call is rejected before execution when cumulative current and prior-resume spend cannot fit the confirmed cap. Provider/model/revision, phase, reserved cost and conservative charged cost are then persisted as generation attempts and surfaced in `/report`. A failed cloud run becomes `needs-attention`; it never masquerades as the open Local draft. `GET /v1/compiles/:id/artifacts`, artifact byte reads and `/report` remain available, while `/resume` requires a newly confirmed higher cap and carries forward recorded spend. Only a completed publication event may replace the open editor world.

...
/** Synchronous browser-safe SHA-256 for deterministic manifest recipe IDs. */
export function sha256Hex(value: string): string {
  ...
  for (let offset = 0; offset < message.byteLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      const previous15 = words[index - 15]!; const previous2 = words[index - 2]!;
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + upper1 + ((e & f) ^ (~e & g)) + roundConstants[index]! + words[index]!) >>> 0;
